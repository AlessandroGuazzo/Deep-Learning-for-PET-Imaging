...
# THE TRAINING SET HAS ONLY miniPET DATA
...
first = True
numSino = 0
for file in glob.glob("Mouse6/*.sino.mnc"):
...
for file in glob.glob("Mouse6/GT.mnc"):
        GTtest = GTprocessing(file,False,0.4,0.1)
        for i in range(numSino-1):
            GTtest = torch.cat((GTtest,GTprocessing(file,False,0.4,0.1)),dim=0)

numSino = 0
for file in glob.glob("Mouse7/*.sino.mnc"):
    if not re.match('Mou.*x1',file):
        testSino = torch.cat((testSino,dataLoad(file)),dim=0)
        numSino = numSino+1

for file in glob.glob("Mouse7/GT.mnc"):
    for i in range(numSino):
        GTtest = torch.cat((GTtest,GTprocessing(file,False,0.4,0.1)),dim=0)
...
for epoch in range(1, epochs + 1): 
    ...
    torch.save({
            ...
            'optimizer_state_dict': optimizer.state_dict(),
                }, 'LPD3miniPETonlyLN'+str(epoch)+'ep.tar')
    print("Model Saved")
