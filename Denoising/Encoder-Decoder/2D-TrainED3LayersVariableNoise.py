...
# THIS CODE CONTINUES OR STARTS THE TRAINING OF AN ENCODER-DECODER FOR PET IMAGE DENOISING 

# THE ENCODER-DECODER HAS 3 LAYERS
...
# ENCODER-DECODER ARCHITECTURE DEFINITION
...
class ED(nn.Module):
    ...
    def forward(self,x):
        ...
        l1 = self.layer1(x)
        #print(l1.shape)
        l2 = self.layer2(self.drop(self.pool1(l1)))
        #print(l2.shape)
        l3 = self.layer3(self.drop(self.pool2(l2)))
        #print(l3.shape)
        
        mid = self.layer4(self.drop(self.pool3(l3)))
        #print(mid.shape)
        
    
        l3up = self.layerUP31(self.drop(mid))
        l3up = self.layerUP32(l3up)
        #print(l3up.shape)
        l4up = self.layerUP41(self.drop(l3up))
        l4up = self.layerUP42(l4up)
        #print(l4up.shape)
        l5up = self.layerUP51(self.drop(l4up))
        l5up = self.layerUP52(l5up)
        #print(l5up.shape)
        out = self.layerUP61(self.drop(l5up))
        #print(out.shape)
...
summary(ED, input_size=(1, 147, 147))
...
def test(): 
    ...
    for batch_idx,(test_im,recons) in enumerate(test_loader):
        loss = loss_test(ED(recons.cuda()), test_im.cuda().float())
...
if use_checkpoint:
    ...
    try:
        ...
        ED.load_state_dict(checkpoint['model_state_dict'])
